...
![工作流程图](./codex_claude_pipeline_flowchart_zh.svg)
...
`codex-call` 是一个 Bash 封装脚本，负责定位 Codex 二进制文件、强制超时（默认 600 秒，可通过 `CODEX_TIMEOUT` 配置）、支持会话复用（`--session-file` / `--resume`）、保存原始输出（`--save-output`）。上下文文件通过 `--file` 标志传递——封装脚本将文件路径注入到 prompt 头部，让 Codex 在只读沙箱中自行读取文件内容，而非由 Claude Code 将文件内容内联到 prompt 中。始终以 `--sandbox read-only` 运行 Codex。
